...
namespace StockProphet_Project.Models {
	...
	public class ModelClass {
		...
		public Prediction MLModeling( List<StockDB> input_untrans ) {
		...
		public List<StockDBA> TransferToInput( List<StockDB> input ) {
			List<StockDBA> output = new List<StockDBA>();

			foreach (var item in input) {
				output.Add(new StockDBA() {
					SB_BussinessIncome = item.SB_BussinessIncome,
					SB_EPS = Convert.ToSingle(item.SB_EPS),
					SB_PBRatio = Convert.ToSingle(item.SB_PBRatio),
					SB_Yield = Convert.ToSingle(item.SB_Yield),
					SI_Dif = Convert.ToSingle(item.SI_Dif),
					SI_D_30 = Convert.ToSingle(item.SI_D_30),
					SI_D_5 = Convert.ToSingle(item.SI_D_5),
					SI_K_30 = Convert.ToSingle(item.SI_K_30),
					SI_K_5 = Convert.ToSingle(item.SI_K_5),
					SI_LongEMA = Convert.ToSingle(item.SI_LongEMA),
					SI_MA = Convert.ToSingle(item.SI_MA),
					SI_MACD = Convert.ToSingle(item.SI_MACD),
					SI_MovingAverage_30 = Convert.ToSingle(item.SI_MovingAverage_30),
					SI_MovingAverage_5 = Convert.ToSingle(item.SI_MovingAverage_5),
					SI_OSC = Convert.ToSingle(item.SI_OSC),
					SI_PE = Convert.ToSingle(item.SI_PE),
					SI_RSV_30 = Convert.ToSingle(item.SI_RSV_30),
					SI_RSV_5 = Convert.ToSingle(item.SI_RSV_5),
					SI_ShortEMA = Convert.ToSingle(item.SI_ShortEMA),
					SN_Code = item.SN_Code,
					SN_Name = item.SN_Name,
					STe_Close = Convert.ToSingle(item.STe_Close),
					STe_Dividend_Year = Convert.ToSingle(item.STe_Dividend_Year),
					STe_Max = Convert.ToSingle(item.STe_Max),
					STe_Min = Convert.ToSingle(item.STe_Min),
					STe_Open = Convert.ToSingle(item.STe_Open),
					STe_SpreadRatio = Convert.ToSingle(item.STe_SpreadRatio),
					STe_TradeMoney = item.STe_TradeMoney,
					STe_TradeQuantity = item.STe_TradeQuantity,
					STe_TransActions = item.STe_TransActions,
					ST_Date = item.ST_Date,
					ST_Quarter = item.ST_Quarter,
					ST_Year = item.ST_Year,
					ST_Year_Quarter = item.ST_Year_Quarter,
					S_PK = item.S_PK
	...
	public class TimeSerialModel {
		...
		public ModelPara MLModeling( List<StockDB> input_untrans, ModelPara mp ) {
